angular.module('JianzhiApp.services',[])
...
.service('User', ['Storage','callApi','$q','$log', function(Storage,callApi,$q,$log){
    ...
    this.getInfo = function(){
        ...
        getInfoDefer = $q.defer();
        callApi.getData('/info/get','POST',null,token)
            .then(function(response){
                getInfoDefer.resolve(response);
            },function(response){
                getInfoDefer.reject(response);
            });
        return getInfoDefer.promise;
    }
    ...
    this.setInfoState = function(content){
        var out_consider = ['experience','intention','picture'];
        for(i in content){
            // console.log(i+content[i])
            if((!content[i]||content[i]==null)&&out_consider.indexOf(i)==-1){
                return;
            }
        }
        infoComplete = true;

...
.controller('AdminjianzhiCtrl', ['$scope','$ionicPopup','$log','$ionicModal','$rootScope','callApi','Admin','MessageShow','$ionicScrollDelegate','$state', function($scope,$ionicPopup,$log,$ionicModal,$rootScope,callApi,Admin,MessageShow,$ionicScrollDelegate,$state){
	...
	var get_detail = function(iid){
		callApi.getData("/record/admin/list","POST",{"iid":iid},user.token)
			.then(function(response){
				console.log(response);
				if(response.code==200){
					...
					MessageShow.MessageShow("获取兼职报名详细失败",1000);
				}
			},function(error){
				MessageShow.MessageShow("获取兼职报名详细失败",1000);
			})
